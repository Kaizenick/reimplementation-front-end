...
const GradeOutOfBoundsChecker: React.FC<{ assignmentId: string | null }> = ({ assignmentId }) => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const { data } = await axiosClient.get(
          `/assignments/${assignmentId}/review_grades_out_of_bounds?${params}`
        );
  ...
  return (
    <div className="alert alert-warning mt-3" role="alert">
      <strong>Warning:</strong> {conflictCount} reviewer{conflictCount > 1 ? "s have" : " has"}{" "}
      already been assigned a grade outside the new scale. Those grades will remain as-is if you
      save.
    </div>
  );
};
...
const AssignmentEditor: React.FC<IEditor> = ({ mode }) => {
  ...
  const {
    data: accessibleDutiesResponse,
    error: accessibleDutiesError,
    sendRequest: fetchAccessibleDuties,
  } = useAPI();
  const {
    data: assignmentDutiesResponse,
    error: assignmentDutiesError,
    sendRequest: fetchAssignmentDuties,
  } = useAPI();
  ...
  const onSubmit = (
    ...
    if (values.is_role_based && id && assignmentDuties.length === 0) {
      dispatch(
        alertActions.showAlert({
          variant: "danger",
          message: "Please add at least one duty when role-based reviews are enabled.",
        })
      );
      return;
    }
  ...
  return (
    <div style={{ padding: "30px", paddingTop: "1.5rem" }}>
      {mode === "update" && (
        <h1 className="text-dark mb-4" style={{ fontSize: "2rem", fontWeight: "600" }}>
                    ...
                    <Table
                      ...
                      data={[
                        ...
                        },
                        {
                          ...
                          questionnaire_options: [
                            {
                              label: "Review with Github metrics",
                              value: "Review with Github metrics",
                            },
                          ],
                          questionnaire_type: "dropdown",
                        },
                      ...
                      >
                        ...
                        {(() => {
                          const assignedIds = new Set(
                            (assignmentDuties || []).map((d: any) => d.id)
                          );
                ...
                <Tab eventKey="etc" title="Etc.">
                  <div className="assignment-actions d-flex flex-wrap justify-content-start">
                    ...
                    <Dropdown className="custom-tab-button" style={{ padding: 0 }}>
                      ...
                      <Dropdown.Menu>
                        <Dropdown.Item
                          onClick={() => navigate(`/assignments/${assignmentData.id}/review`)}
                        >
                          Review Report
                        </Dropdown.Item>
                        <Dropdown.Item
                          onClick={() =>
                            navigate(`/assignments/${assignmentData.id}/teammate-review`)
                          }
                        >
              ...
              {(formik) => {
                return (
                  <Form>
                    ...
                    />
                    <FormCheckbox
                      controlId="assignment-availability_flag"
