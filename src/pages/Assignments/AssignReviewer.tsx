...
interface ResponseMapRow {
  ...
  reviewee_team_id?: Id | null;
  reviewer_user_id?: Id | null;
}
...
function keyFor(asgId: Id) {
  return `assignreviewer:${asgId}`;
}
function read(asgId: Id): Persist | null {
  try {
    const s = localStorage.getItem(keyFor(asgId));
    return s ? (JSON.parse(s) as Persist) : null;
  } catch {
    return null;
  }
}
function write(asgId: Id, p: Persist) {
  localStorage.setItem(keyFor(asgId), JSON.stringify(p));
}
...
function toView(u?: User | null, fallbackId?: Id): IUserView | undefined {
  if (u)
    return {
      ...
      fullName: u.full_name ?? u.name ?? `user_${u.id}`,
    };
  if (fallbackId !== undefined)
    return { id: fallbackId, username: `user_${fallbackId}`, fullName: `user_${fallbackId}` };
  return undefined;
}

function isArr<T>(x: any): x is T[] {
  ...
  const safe: Persist = {
    assignment:
      raw?.assignment && typeof raw.assignment === "object"
        ? {
            id: Number(raw.assignment.id) || asgId,
            name: String(raw.assignment.name ?? ASG_NAME?.[asgId] ?? `Assignment ${asgId}`),
          }
...
export function demo(asgId: Id): Persist {
  ...
  const membersOf = (tid: number) =>
    teams_users
      .filter((tu) => tu.team_id === tid)
      .map((tu) => users.find((u) => u.id === tu.user_id)!) as User[];
...
const AssignReviewer: React.FC = () => {
  ...
  const { assignment, teams, users, teams_users, participants, response_maps, responses } =
    persisted;
  ...
  const participantsById = useMemo(
    () => new Map<Id, Participant>(participants.map((p) => [p.id, p])),
    [participants, tick]
  );
  ...
  const latestResponseByMap = useMemo(() => {
    ...
    responses.forEach((r) => {
      ...
      const prevTs = prev
        ? new Date((prev.updated_at ?? prev.created_at ?? "") as string).getTime() || 0
        : -1;
  ...
  const statusForMap = (mapId: Id): ReviewStatus => {
    ...
    const submitted =
      typeof latest.is_submitted === "boolean" ? latest.is_submitted : latest.is_submitted === 1;
  ...
  const rows: ITeamRow[] = useMemo(() => {
    ...
    return teamIds.map<ITeamRow>((teamId) => {
      ...
      const mentor = t?.mentor_id
        ? toView(usersById.get(t.mentor_id) ?? null, t.mentor_id)
        : undefined;
  ...
  function onAddReviewer(teamId: number) {
    ...
    if (!Number.isFinite(reviewerUserId)) {
      window.alert("Invalid user_id.");
      return;
    }

    mutate((p) => {
      let reviewerPart = p.participants.find(
        (x) => x.user_id === reviewerUserId && x.parent_id === assignmentId
      );
      if (!reviewerPart) {
        ...
        if (!p.users.find((u) => u.id === reviewerUserId)) {
          p.users.push({
            id: reviewerUserId,
            name: `user_${reviewerUserId}`,
            full_name: `user_${reviewerUserId}`,
          });
        }
      }
    ...
    mutate((p) => {
      const ids = new Set<Id>(
        p.response_maps
          .filter(
            (m) =>
              m.reviewed_object_id === assignmentId &&
              (m.reviewee_team_id === teamId || m.reviewee_id === teamId)
          )
          .map((m) => m.id)
      );
  ...
  const empty =
    ...
    response_maps.length === 0;

  return (
    <Container fluid className="px-3">
      <div className="ex-shell">
        <div className="ex-help mb-3">
          Assign Reviewers — {hasValidId ? assignment?.name : "Assignment"}{" "}
          {hasValidId ? `(ID: ${assignmentId})` : "(ID: unknown)"} · teams:{teams.length} · maps:
          {response_maps.length} · responses:{responses.length}
        </div>

        {!hasValidId && (
          <div
            className="ex-help mb-3"
            style={{ color: "#a94442", background: "#f2dede", border: "1px solid #ebccd1" }}
          >
        ...
        <Row className="align-items-center mb-2 g-2">
          <Col xs={12} md className="min-w-0">
            <h1 className="m-0 text-truncate">
              Assign Reviewer — {hasValidId ? assignment?.name : "Assignment"}{" "}
              {hasValidId ? `(ID: ${assignmentId})` : ""}
            </h1>
          </Col>
          <Col xs="auto" className="d-flex align-items-center gap-3 flex-shrink-0">
            ...
            <Button
              ...
              disabled={
                !hasValidId || !empty ? false : false /* still enabled to seed when empty */
              }
              onClick={() => {
                if (!hasValidId) return;
                if (!empty) {
                  window.alert("Data exists. Clear first to load demo.");
                  return;
                }
                ...
                <tr>
                  <td className="ex-cell" colSpan={2}>
                    <span className="ex-muted">
                      No reviewer data to display. Use “Load demo data” or add reviewers after you
                      add teams/users locally.
                    </span>
                  </td>
                </tr>
              )}
                  ...
                  <td className="ex-cell">
                    ...
                    <div className="ex-actions">
                      <a
                        role="button"
                        className="ex-link"
                        onClick={() => hasValidId && onAddReviewer(team.id)}
                      >
                        add reviewer
                      </a>
                      <a
                        role="button"
                        className="ex-link"
                        onClick={() => hasValidId && onDeleteAll(team.id)}
                      >
                    ...
                    {team.reviewers.map((r) => (
                      <div key={r.id} className="ex-review-row" data-testid="ex-review-row">
                        ...
                        {r.status === "Submitted" && (
                          <a
                            role="button"
                            className="ex-inline-link"
                            onClick={() => hasValidId && onUnsubmit(team.id, r.id)}
                          >
                            (unsubmit)
                          </a>
                        )}
                        <a
                          role="button"
                          className="ex-inline-link"
                          onClick={() => hasValidId && onDeleteReviewer(team.id, r.id)}
                        >
                          delete
                        </a>
                      </div>
