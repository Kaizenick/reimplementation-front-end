...
const ReviewTableau: React.FC = () => {
  ...
  console.log("ReviewTableau render - params", {
    reviewerId,
    assignmentId,
    participantId,
    isLoading,
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (!assignmentId || !participantId) {
        setError(
          "Unauthorized: Missing required parameters. Please provide assignmentId and participantId in the URL."
        );
        return;
      }
      ...
      try {
        console.log("ReviewTableau: requesting review tableau data", {
          assignmentId,
          participantId,
        });
  ...
  const renderReviewCell = (item: RubricItem, response: any) => {
    ...
    if (
      (questionType === "Scale" || questionType === "Criterion" || item.itemType === "Criterion") &&
    ...
    if (
      ...
      (response.selectedOption !== undefined || response.score !== undefined)
    ) {
    ...
    if (questionType === "Checkbox" || item.itemType === "Checkbox") {
      ...
      if (checkValue === undefined && response.selectedOption !== undefined) {
        ...
        checkValue =
          ...
          response.selectedOption === "true" ||
          response.selectedOption === true;
      }

      if (checkValue !== undefined) {
        const isChecked =
          checkValue === true || checkValue === 1 || checkValue === "1" || checkValue === "true";
        return (
    ...
    if (
      (questionType === "TextArea" || questionType === "TextField") &&
      response.textResponse !== undefined
    ) {
      return <div className="text-response-cell">{response.textResponse || "—"}</div>;
    }
  ...
  return (
    <div className="review-by-student-container">
      ...
      <div className="course-info">
        ...
        <div>
          <strong>Assignment:</strong> {data.assignment}
        </div>
      </div>
      ...
      {roundRubricGroups.map((roundGroup) => (
        <div key={roundGroup.roundNumber} className="round-section">
          ...
          {roundGroup.rubricRounds.map((rubricRound) => {
            ...
            return (
              <div
                key={`${roundGroup.roundNumber}_${rubricRound.rubric.id}`}
