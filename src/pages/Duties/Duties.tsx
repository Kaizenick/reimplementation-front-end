...
const Duties = () => {
  ...
  const [showDelete, setShowDelete] = useState<{ visible: boolean; duty?: IDuty }>({
    visible: false,
  });
  ...
  const sortWithMineOnTop = useCallback(
    (rows: IDuty[]) => {
  ...
  const tableData = useMemo(() => {
    ...
    const filteredDuties = (allDuties || []).filter(
      (d) => !d.private || d.instructor_id === userId
    );
  ...
  return (
    <>
          ...
          <Row>
            <Col md={{ span: 1, offset: 11 }} style={{ paddingBottom: "10px" }}>
              <Button
                ...
                onClick={() => setShowEditor({ visible: true, mode: "create" })}
              >
                +
              </Button>
            </Col>
      ...
      {showDelete.visible && showDelete.duty && (
        <DutyDelete
          duty={{ id: showDelete.duty.id, name: showDelete.duty.name }}
