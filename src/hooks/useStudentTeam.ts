...
export const useStudentTeam = (studentId: string) => {
  ...
  const fetchSentInvitationsByTeam = (teamId: number) =>
    fetchSentInvitationsByTeamAPI.sendRequest({
      url: `/invitations/sent_by/team/${teamId}`,
    });

  const fetchSentInvitationsByParticipant = (participantId: number) =>
    fetchSentInvitationsByParticipantAPI.sendRequest({
      url: `/invitations/sent_by/participant/${participantId}`,
    });

  const fetchReceivedInvitations = () =>
    fetchReceivedInvitationsAPI.sendRequest({
      url: `/invitations/sent_to/${studentId}`,
    });

  const fetchJoinTeamRequests = (teamId: number) =>
    fetchJoinTeamRequestsAPI.sendRequest({
      url: `/join_team_requests/for_team/${teamId}`,
    });

  const acceptJoinRequest = (requestId: number) =>
    acceptJoinRequestAPI.sendRequest({
      url: `/join_team_requests/${requestId}/accept`,
      method: "PATCH",
    });

  const declineJoinRequest = (requestId: number) =>
    declineJoinRequestAPI.sendRequest({
